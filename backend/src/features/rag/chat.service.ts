...
export class ChatService {
  ...
  /**
   * Ask a question and store in chat history (with conversation support)
   */
  async ask(
    userId: string,
    question: string,
    ...
    // Get answer using RAG (with conversation context)
    const ragResponse = await this.ragService.ask(
      question,
      documentIds,
      5,
      conversationId,
